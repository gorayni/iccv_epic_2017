...
def filtered_vgg_16_plus_lstm_first_phase(weights=None, timestep=10, img_width=224, img_height=224):
    ...
    lstm_output = LSTM(256, return_sequences=True, name='lstm1')(x)
    x = TimeDistributed(Dropout(0.5))(lstm_output)
...
def filtered_vgg_16_plus_lstm_second_phase(weights=None, timestep=10, img_width=224, img_height=224):
    input_shape = (timestep, img_width, img_height, 3)
    ...
    # Classification block
    x = TimeDistributed(Flatten(), name='flatten')(x)
    x = TimeDistributed(Dense(4096, activation='relu', kernel_constraint=maxnorm(2.), trainable=False), name='fc1')(x)
    x = TimeDistributed(Dropout(0.5))(x)
    x = TimeDistributed(Dense(256, activation='relu'), name='fc2_')(x)
    x = TimeDistributed(Dropout(0.5))(x)
...
def filtered_vgg_16_plus_lstm(weights=None, timestep=10, img_width=224, img_height=224):
    ...
    lstm_output = LSTM(256, return_sequences=True, name='lstm1')(x)
    x = TimeDistributed(Dense(21, activation='softmax'), name='predictions_')(lstm_output)
